...
type InMemoryClient struct {
	store map[Info]*actuation.Inventory
}
...
func (imc *InMemoryClient) Load(_ context.Context, invInfo Info) (*actuation.Inventory, error) {
...
func (imc *InMemoryClient) Delete(_ context.Context, invInfo Info, dryRun common.DryRunStrategy) error {
	if dryRun.ClientOrServerDryRun() {
...
func (imc *InMemoryClient) lazyInit() {
	if imc.store == nil {
		imc.store = make(map[Info]*actuation.Inventory)
	}
}
