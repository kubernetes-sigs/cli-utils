...
func (cc *ClusterClient) Load(ctx context.Context, invInfo Info) (*actuation.Inventory, error) {
...
func (cc *ClusterClient) Delete(ctx context.Context, invInfo Info, dryRun common.DryRunStrategy) error {
	infoGK := GroupKindFromObjectReference(invInfo.ObjectReference)
	gvk := cc.GroupVersionKind()
	gk := gvk.GroupKind()
	if infoGK != gk {
...
func (cc *ClusterClient) updateObject(
	...
	if cc.StatusPolicy == StatusPolicyAll && obj != nil && out != nil {
		...
		if err != nil {
			return out, fmt.Errorf("failed to update object status %q: %w", id, err)
		}
	}
