...
// ApplyDefaults sets the default namespace for source references to the
// namespace of the parent target object.
func (atm *ApplyTimeMutator) ApplyDefaults(objs []*unstructured.Unstructured) error {
	for _, obj := range objs {
		err := atm.applyDefaultsInner(obj)
		if err != nil {
			targetRef := mutation.NewResourceReference(obj)
...
func (atm *ApplyTimeMutator) applyDefaultsInner(obj *unstructured.Unstructured) error {
	subs, err := mutation.ReadAnnotation(obj)
	if err != nil {
		return err
	}
	newSubs, err := atm.applyDefaultNamespace(subs, obj.GetNamespace())
	if err != nil {
		return err
	}
...
func (atm *ApplyTimeMutator) applyDefaultNamespace(subs mutation.ApplyTimeMutation, namespace string) (mutation.ApplyTimeMutation, error) {
	if namespace == "" {
		return subs, nil
	}
	...
	for i, sub := range subs {
		// lookup REST mapping
		sourceMapping, err := atm.getMapping(sub.SourceRef)
		if err != nil {
		...
		if sub.SourceRef.Namespace == "" && sourceMapping.Scope.Name() == meta.RESTScopeNameNamespace {
			...
			klog.V(5).Infof("applied default namespace: %#v", sub.SourceRef)
		}
		newSubs[i] = sub
	}
	return newSubs, nil
