...
func (i *DeleteOrUpdateInvTask) updateInventory(taskContext *taskrunner.TaskContext) error {
	...
	prevInventory := i.Inventory.ObjectRefs()
	...
	if i.StatusPolicy == inventory.StatusPolicyAll {
		...
		if err := i.InvClient.UpdateStatus(taskContext.Context(), i.Inventory, inventory.UpdateOptions{}); err != nil {
			return err
		}
	}
...
func (i *DeleteOrUpdateInvTask) deleteInventory(ctx context.Context) error {
	...
	err := i.InvClient.Delete(ctx, i.Inventory, inventory.DeleteOptions{})
