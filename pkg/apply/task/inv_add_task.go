...
func (i *InvAddTask) Start(taskContext *taskrunner.TaskContext) {
	go func() {
		...
		if inv.GetNamespace() != "" {
			if invNamespace := NamespaceInSet(inv, i.ApplyObjects); invNamespace != nil {
...
// NamespaceInSet returns the the namespace the passed inventory
...
func NamespaceInSet(inv client.Object, objs object.UnstructuredSet) *unstructured.Unstructured {
	invID := inventory.Label(inv)
