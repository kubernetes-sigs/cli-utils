...
func dependsOnWithExternalDependencyTest(c client.Client, invConfig InventoryConfig, inventoryName, namespaceName string) {
	By("create pod2 manually")
	pod2Unstructured := withNamespace(manifestToUnstructured(pod2), namespaceName)
	err := c.Create(context.TODO(), pod2Unstructured)
	Expect(err).NotTo(HaveOccurred())
	...
	// Dependency order: pod3 -> pod2
	// Apply order: pod2 (pre-applied), pod3
	resources := []*unstructured.Unstructured{
		withDependsOn(withNamespace(manifestToUnstructured(pod3), namespaceName), fmt.Sprintf("/namespaces/%s/Pod/pod2", namespaceName)),
	}
	...
	By("delete pod2 manually")
	deleteUnstructuredAndWait(c, pod2Unstructured)
}
