...
func applyWithExistingInvTest(ctx context.Context, c client.Client, invConfig invconfig.InventoryConfig, inventoryName, namespaceName string) {
	...
	orgApplyInv := inventory.InfoFromObject(invConfig.FactoryFunc(inventoryName, namespaceName, orgInventoryID))
	...
	secondApplyInv := inventory.InfoFromObject(invConfig.FactoryFunc(inventoryName, namespaceName, secondInventoryID))
